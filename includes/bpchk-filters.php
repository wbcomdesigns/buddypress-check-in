...
add_action('bp_after_activity_post_form','bpchk_show_locations_and_google_map_in_activity',10,3);
function bpchk_show_locations_and_google_map_in_activity()
{
	...
	{
		wp_enqueue_script('bpchk-js-map-auto-complete', 'http://maps.googleapis.com/maps/api/js?key=AIzaSyAaB5dGHZoDvmFAlcJoN-JmtoCB-wiAIzM&libraries=places&sensor=false', array('jquery'));		
	...
	{
		wp_enqueue_script('bpchk-js-map-place-types', 'http://maps.googleapis.com/maps/api/js?key=AIzaSyAaB5dGHZoDvmFAlcJoN-JmtoCB-wiAIzM&libraries=places&sensor=false',array('jquery'));
...
add_filter('single_template', 'my_custom_post_template');

function my_custom_post_template($single) {
    global $wp_query, $post;
    /* Checks for single template by post type */
    if ($post->post_type == "place"){
        if(file_exists(BPCHK_PLUGIN_PATH . 'single-place.php'))
            return BPCHK_PLUGIN_PATH . 'single-place.php';
    }
    return $single;
}
?>